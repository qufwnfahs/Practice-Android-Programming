...
class Fragment_Practice : Fragment(), PracticeRVAdapter.OnItemClickListener {

    internal lateinit var view : View
    var practiceItems = arrayListOf<PracticeItem>()
    val margin = 20
    ...
    fun setItems() {
        var recyclerView = view.recyclerView_practice
        ...
        for (item in texts) {
            practiceItems.add(PracticeItem(item, margin))
        }

        var recyclerAdapter = PracticeRVAdapter(practiceItems, recyclerView)
    ...
    override fun onItemClick(position: Int) {
        ...
        intent.putExtra("SubTitle", practiceItems[position].text)
